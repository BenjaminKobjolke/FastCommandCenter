...
`build_tray(app, open_palette, open_settings)` (`gui/tray.py`) creates:
...
- **Context menu** (`QMenu`) — four entries, in order:
  ...
  2. `Settings` → `open_settings` (opens `SettingsDialog`, modal via `exec()`)
  3. *(separator)*
  4. `Quit` → `app.quit`

`main()` calls `build_tray(app, open_palette, open_settings)` after the hotkey
manager is started, and does **not** need to keep the returned value — the
tray keeps itself alive (below).
...
- `app.setQuitOnLastWindowClosed(False)` (`fastcommandcenter.py`) is required
  because this is a background app: closing the Settings dialog is "the last
  window" from Qt's point of view, and without this flag the whole app would
  exit when Settings closes.
